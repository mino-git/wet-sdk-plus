...
static void WM_DrawClientScore( int x, int y, score_t *score, float *color, float fade ) {
	...
	if ( score->client == cg.snap->ps.clientNum ) {
		// sta acqu-sdk (issue 2): CHRUKER: b077
		//tempx = x;
		// end acqu-sdk (issue 2): CHRUKER: b077

		hcolor[3] = fade * 0.3;
		VectorSet( hcolor, .5f, .5f, .2f );			// DARK-RED

		// sta acqu-sdk (issue 2): CHRUKER: b077 - Player highlighting was split into columns
	...
	if ( ci->team != TEAM_SPECTATOR ) {
		if ( ci->powerups & ( (1 << PW_REDFLAG) | (1 << PW_BLUEFLAG) ) ) {
			// sta acqu-sdk (issue 2): CHRUKER: b078 - Medic, death and objective icons on the scoreboard are drawn too big
			CG_DrawPic( tempx - 3, y + 1, 14, 14, cgs.media.objectiveShader );
			//CG_DrawPic( tempx-4, y, 16, 16, cgs.media.objectiveShader );
			// end acqu-sdk (issue 2): CHRUKER: b078
			// sta acqu-sdk (issue 2): CHRUKER: b072/b078 - Need to match tempx or else the other text gets offset
			offset += 14;
			//offset += 8;
			// end acqu-sdk (issue 2): CHRUKER: b072
			// sta acqu-sdk (issue 2): CHRUKER: b078
			tempx += 14;
			//tempx += 12;
			// end acqu-sdk (issue 2): CHRUKER: b078
			maxchars -= 2;
...
static void WM_DrawClientScore_Small( int x, int y, score_t *score, float *color, float fade ) {
	...
	if ( ci->team != TEAM_SPECTATOR ) {
		if ( ci->powerups & ( (1 << PW_REDFLAG) | (1 << PW_BLUEFLAG) ) ) {
			// sta acqu-sdk (issue 2): CHRUKER: b071 - Objective carrier icon missing on compact scoreboard
			// sta acqu-sdk (issue 2): CHRUKER: b078 - Medic, death and objective icons on the scoreboard are drawn too big
			...
			// end acqu-sdk (issue 2): CHRUKER: b071
			offset += 10;
			tempx += 10;
			//offset += 14;
			//tempx += 14;
...
// end acqu-sdk (issue 2): CHRUKER: b035
	...
	// sta acqu-sdk (issue 2): CHRUKER: b076 - Adjusted y coordinate, and changed to use DrawBottom instead of DrawTopBottom
	...
	// sta acqu-sdk (issue 2): CHRUKER: b076
	...
	// sta acqu-sdk (issue 2): CHRUKER: b076 - The math says char height + 2 * border width (1 pixel)
	y += SMALLCHAR_HEIGHT + 2;
	//y += SMALLCHAR_HEIGHT;
	// end acqu-sdk (issue 2): CHRUKER: b076
	...
	for ( i = 0; i < maxrows; i++ ) {
		...
		if ( use_mini_chars ) {
			// sta acqu-sdk (issue 2): CHRUKER: b076 - Adjusted y height, and changed to DrawBottom instead of DrawTopBottom
